...
public class ChordProtocols {
	...
	/**
	 * UpdateSuccessor StabilizeRing FixFingerTable CheckPredecessor
	 */
		...
		try {
			...
			if (registry != null) {
				try {
					...
					// call remote findSuccessor function. The result will be the successor of this
					// randomNode
					NodeInterface chordnodeSuccessor = randomNode.findSuccessor(chordnode.getNodeID());

					// set the successor of this node to chordnodeSuccessor and its predecessor to
					// null
	...
	private void createRing(Node node) throws RemoteException {
		...
		logger.info("New ring created. Node = " + node.getNodeName() + " | Successor = "
				+ node.getSuccessor().getNodeName() + " | Predecessor = " + node.getPredecessor());
			...
			if (succnode != null) { // add chordnode's keys to its successor
				keyids.forEach(fileID -> {
					try {
						...
						succnode.saveFileContent(msg.getNameOfFile(), fileID, msg.getBytesOfFile(),
	...
	public void fixFingerTable() {
		...
		try {
			...
			fingertable.clear();
			// get the address size from the Hash class. This is the modulus and our address
			// space (2^mbit = modulus)
			BigInteger addressSize = Hash.addressSize();
			// get the number of bits from the Hash class. Number of bits = size of the
			// finger table
			int m = Hash.bitSize();
			...
			for (int i = 0; i < m; i++) {
				...
				BigInteger pow = BigInteger.TWO.pow(i).mod(addressSize);
				BigInteger k = chordnode.getNodeID().add(pow).mod(addressSize);
				// then: use chordnode to find the successor of k. (i.e., succnode =
				// chordnode.findSuccessor(k))
